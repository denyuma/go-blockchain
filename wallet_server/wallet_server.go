...
func (ws *WalletServer) Wallet(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		log.Println("ERROR : Invalud HTTP Method")
	}
}
...
func (ws *WalletServer) WalletAmount(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		client := &http.Client{}
		bscReq, _ := http.NewRequest("GET", endpoint, nil)
		q := bscReq.URL.Query()
		q.Add("blockchain_address", blockchainAddress)
		bscReq.URL.RawQuery = q.Encode()

		bcsResp, err := client.Do(bscReq)
		if err != nil {
			log.Printf("ERROR: %v", err)
			io.WriteString(w, string(utils.JsonStatus("fail")))
			return
...
func (ws *WalletServer) Run() {
	...
	log.Fatal(http.ListenAndServe("0.0.0.0:0"+strconv.Itoa(int(ws.Port())), nil))
}
